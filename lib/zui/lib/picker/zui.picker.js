...
(function($, window, document) {
    ...
    var Picker = function(element, options) {
        ...
        if (list) {
            that.setList(typeof list === 'function' ? list({
                search: that.search,
            ...
            if (data) {
                if ($.isPlainObject(data)) {
                    if ((data.result === 'success' || data.result === 'ok') && $.isArray(data.data)) {
                        ...
                        that.updateMessage(JSON.stringify(data), 'danger');
                    } else {
                        var dataList = [];
    ...
    Picker.prototype.showDropList = function() {
        ...
        if (that.triggerEvent('showingDrop', {
                picker: that
            }) === false) {
            return;
        }
    ...
    Picker.prototype.hideDropList = function() {
        ...
        if (that.triggerEvent('hidingDrop', {
                picker: that
            }) === false) {
            return;
        }
        ...
        $.zui({
            ...
            fixBodyScrollbar: function() {
                if ($.zui.checkBodyScrollbar()) {
                    ...
                    if ($.zui._scrollbarWidth) {
                        $body.css({
                            paddingRight: bodyPad + $.zui._scrollbarWidth,
                            overflowY: 'hidden'
                        });
                    }
                    return true;
