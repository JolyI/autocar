...
(function($, undefined) {
        ...
        if (dataSource.array) {
            ...
            if (!loadData && remote) {
                loadData = function(params, onFinish) {
                    var ajaxOptions = $.isFunction(remote) ? remote(params, that) : {
                        url: remote
                    };
                    $.ajax($.extend({
                        type: 'GET',
                        ...
                        success: function(responseData, textStatus, jqXHR) {
                            ...
                            if ($.isPlainObject(responseData) && responseData.data) {
                                ...
                                if (result === 'success' || result === 'ok' || result === 200) {
                                    ...
                                    onFinish(false, response responseData.reason || that.lang['errorCannotHandleRemoteData'], responseData);
                                }
                            } else {
    ...
    DataGrid.prototype.scroll = function(scrollLeft, scrollTop, ignoreDelay) {
        ...
        that.$.callComEvent(that, 'onScroll', [scrollLeft, scrollTop, {
            vScrolled: vScrolled,
            hScrolled: hScrolled
        }]);
    };
